...
if [ -n "$IS_UPDATE" ]; then
  ...
  # workspaces hold generated work that exists nowhere else and .venv is expensive to
  # rebuild, so both are excluded from the sync rather than merely "probably not touched".
  if need rsync; then
    rsync -a --delete \
      --exclude 'webapp/workspaces/' --exclude 'webapp/.venv/' \
