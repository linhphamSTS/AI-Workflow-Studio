#!/usr/bin/env python3
"""Cross-platform deploy script for the linhpham-diagram skill.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deploy linhpham-diagram skill to all Claude profiles.")
    parser.add_argument("--mode", choices=["link", "copy"], default="link")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--profile", action="append")
    args = parser.parse_args()

    print(f"= linhpham-diagram deploy ({args.mode} mode) =")
