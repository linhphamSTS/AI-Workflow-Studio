...
def main():
    ...
    sheet_names = [s['name'][:31] for s in spec['sheets']]
    ...
    for name in sheet_names:
        ...
        ck.add('%s: section rows carry no hours' % tag,
               all(ws.cell(r, c).value is None
                   for r in sec_rows for c in range(first_eff, total_col + 1)),
               '%s has a value on a section row, which would double-count' % tag)
        ...
        if total_row:
            ck.add('%s: TOTAL sums the body, not the subtotals' % tag,
                   all(ws.cell(total_row, c).value ==
                       '=SUM(%s3:%s%d)' % (get_column_letter(c), get_column_letter(c),
                                           total_row - 1)
